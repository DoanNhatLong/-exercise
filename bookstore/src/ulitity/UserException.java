...
//
    ...
    public static void validateDate(String dateStr) throws UserException {
        try {
            String[] parts = dateStr.split("/");
            if (parts.length != 3) {
                throw new UserException(ERROR_DATE);
...
//    }
    ...
    public static void validateVolume(int input) throws UserException {
        if (input < 1) {
            throw new UserException(ERROR_INPUT);
        }
    }

    public static void validateIsbn(String isbn) throws UserException {
        if (!Pattern.matches("[\\d-]{10,17}", isbn)) {
            throw new UserException("Mã không hợp lệ. Định dạng KH-YYYY.");
        }
    }
}
